...
int main(void)
{
	...
	cio_print("nRF2401 v0.1 - TestServer ESBPL\n\r");
	...
	while (1) {
		cio_printf("Type character to send. ");
		ptx.data[0] = cio_getc();

		cio_printf("Sending: %c ", ptx.data[0]);

		s = nrf_send_blocking(&ptx);

		if(s == NRF_ERR_MAX_RT) {
			cio_printf("- Unable to send message (MAX_RT)\n\r");
		}
		else if(s == NRF_ERR_TX_FULL) {
			cio_printf("- Unable to send message (TX_FULL)\n\r");
		}
		else {
			...
			if(s == 0) {
				cio_print("- No ACK payload received\n\r");
			}
			else {
				cio_printf("- ACK payload received: %u\n\r", prx.data[0]);
			}
		}
	}
